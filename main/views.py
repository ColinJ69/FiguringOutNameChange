...
from main.forms import RegisterForm, points_form, address_form
...
def games(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            points_added = form.cleaned_data['points']
    ...
    final_scores = zip(top_names, top_scores)
    print(top_names)
    
    return render(request, 'games.html', {'form': form, 'top_scores': final_scores})
    ...
    if request.method == 'POST':
        ...
        for models in points_model.objects.all():
            ...
            if models.user == request.user.username:
                ...
                models.save()
                return HttpResponse(models.points, content_type="application/json")
